---
title: "Summarize MVP MtDNA Analysis results"
...
## Load Info
- Summary of PheCodes in each population
```{r, message=F}
...
- Supplementary Table 1: cardio-metabolic PheCodes 
```{r}
...
- Load annotation file provided by Genisis support and annotations extracted from Hmtvar database
```{r}
...
- Join PheCodes info and add SNP annotation
```{r}
d_snp_all <- d_snp_all %>% 
...
- Output all reporeted PheCodes
```{r, eval = F}
# output all report phecodes
...
### Figure 2: Forestplot in the manuscript (Female vs Male)
- SNP-based interaction results 
```{r}
#(Sex)
...
intx_t2d <- read_xlsx("../SNPbased/interaction_sex_t2d/intx_result_t2d_v2_glm.xlsx", 
                      col_types = c(rep("guess", 13), "text", rep("guess", 4)))
```
- Include all interaction tests whether significant or not 
```{r}
...
- Read in summary level results and clean 
```{r, message=F}
...
## Compare with gnomAD 
```{r}
suppl_table3 <- read_xlsx(paste(manu_supp_dir, "SupplementaryTables10032023.xlsx", sep = ""), 
                          sheet = "Suppl. Table 3") 
suppl_table3 <- suppl_table3[-1, ]
names(suppl_table3) <- suppl_table3[1, ]
suppl_table3 <- suppl_table3[-1, -1*seq(31, 42)]

mvp_AF <- suppl_table3 %>% select(BP, EUR, AMF, HIS, EAS) %>%
...
gnomAD_sum <- read_xlsx(paste(manu_supp_dir, "table_gnomAD_revised.xlsx", sep = "")) %>% 
  group_by(BP) %>%
  mutate(in_mvp = ifelse(sum(is.na(`EUR Homoplasmic AF`), is.na(`EUR Heteroplasmic AF`),
                         is.na(`AMF Homoplasmic AF`), is.na(`AMF Heteroplasmic AF`),
                         is.na(`AFR Homoplasmic AF`), is.na(`AFR Heteroplasmic AF`),
                         is.na(`EAS Homoplasmic AF`), is.na(`EAS Heteroplasmic AF`)) == 8, 0, 1))

gnomAD_sum <- gnomAD_sum %>% 
  left_join(mvp_AF, by = "BP") %>%
  mutate_at(vars("EUR Homoplasmic AF", "EUR Heteroplasmic AF", 
                 "AMF Homoplasmic AF", "AMF Heteroplasmic AF",
                 "AFR Homoplasmic AF", "AFR Heteroplasmic AF",
                 "EAS Homoplasmic AF", "EAS Heteroplasmic AF",
                 "EUR", "AMF", "AFR", "EAS"), ~ifelse(is.na(.) |.<1e-6, NA, -log10(.))) 
...
```{r}
library(ggrepel)
dat <- gnomAD_sum %>% select(BP, `EUR Homoplasmic AF`, EUR) %>% 
  filter(!is.na(`EUR Homoplasmic AF`)) %>% 
  arrange(BP) %>%
  mutate(`EUR Homoplasmic AF` = as.numeric(`EUR Homoplasmic AF`),
         EUR = as.numeric(EUR),
         BP = paste("MT", BP, sep = ""),
         `MAF < 0.1%` = ifelse(EUR > 3, "Yes", "No"),
         diff = abs(`EUR Homoplasmic AF`-EUR),
         Name = ifelse(EUR <= 3 & diff > 0.5, BP, "")) %>%
  rename(gnomAD = `EUR Homoplasmic AF`, 
         MVP = EUR,
         `Absolute difference in MAF` = diff) 

ggplot(dat, aes(MVP, `Absolute difference in MAF`, 
                       color = `MAF < 0.1%`, size = `Absolute difference in MAF`)) +            
  geom_point() + 
  xlab("-log10(MAF in MVP)") + 
  ylab("Absolute difference in MAF") +
...
ggsave("compare_MAF_gnomAD_EUR.pdf", width = 20, height = 10, units = "cm")
```


```{r}

ggp <- ggplot(gnomAD_sum, aes(`EUR Homoplasmic AF`, EUR)) +            
  geom_point(size = 2) 
 

rr = signif(cor(gnomAD_sum$`EUR Homoplasmic AF`, gnomAD_sum$EUR, 
                                use = "pairwise.complete.obs"), 3)
tm.lm = summary(lm(EUR~ -1 + `EUR Homoplasmic AF`, data = gnomAD_sum))
...
ggp +                                      
  ...
  annotate(geom = "text", x = 3, y = 1, 
              ...
              color="red",
           size = 5) + 
  annotate(geom = "text", x = 3, y = 0.7,
           label = paste("Slope = ", slope, sep = ""),
           color= "red",
           size = 5) +
  theme_classic() 
ggsave("cor_MAF_gnomAD_EUR.pdf", width = 20, height = 10, units = "cm")
```
```{r}
library(ggrepel)
dat <- gnomAD_sum %>% select(BP, `AFR Homoplasmic AF`, AFR) %>% 
  filter(!is.na(`AFR Homoplasmic AF`)) %>% 
  arrange(BP) %>%
  mutate(`AFR Homoplasmic AF` = as.numeric(`AFR Homoplasmic AF`),
         AFR = as.numeric(AFR),
         BP = paste("MT", BP, sep = ""),
         `MAF < 0.1%` = ifelse(AFR > 3, "Yes", "No"),
         diff = abs(`AFR Homoplasmic AF` - AFR),
         Name = ifelse(AFR <= 3 & diff > 1.5, BP, "")) %>%
  rename(gnomAD = `AFR Homoplasmic AF`, 
         MVP = AFR,
         `Absolute difference in MAF` = diff) 
...
ggsave("compare_MAF_gnomAD_AFR.pdf", width = 20, height = 10, units = "cm")
```

```{r}

ggp <- ggplot(gnomAD_sum, aes(`AFR Homoplasmic AF`, AFR)) +            
  geom_point(size = 2) 
 

rr = signif(cor(gnomAD_sum$`AFR Homoplasmic AF`, gnomAD_sum$AFR, 
                                use = "pairwise.complete.obs"), 3)
tm.lm = summary(lm(AFR~ -1 + `AFR Homoplasmic AF`, data = gnomAD_sum))
slope = signif(tm.lm$coefficients[1,1], 3)
...
ggp +                                      
  ...
  annotate(geom = "text", x = 3, y = 0.7,
           ...
           size = 5) +
  theme_classic() 
ggsave("cor_MAF_gnomAD_AFR.pdf", width = 20, height = 10, units = "cm")
```
```{r}
library(ggrepel)
dat <- gnomAD_sum %>% select(BP, `AMF Homoplasmic AF`, AMF) %>% 
  filter(!is.na(`AMF Homoplasmic AF`)) %>% 
  arrange(BP) %>%
  mutate(`AMF Homoplasmic AF` = as.numeric(`AMF Homoplasmic AF`),
         AMF = as.numeric(AMF),
         BP = paste("MT", BP, sep = ""),
         `MAF < 0.1%` = ifelse(AMF > 3, "Yes", "No"),
         diff = abs(`AMF Homoplasmic AF` - AMF),
         Name = ifelse(AMF <= 3 & diff > 1.5, BP, "")) %>%
  rename(gnomAD = `AMF Homoplasmic AF`, 
         MVP = AMF,
         `Absolute difference in MAF` = diff) 
...
ggsave("compare_MAF_gnomAD_AMR.pdf", width = 20, height = 10, units = "cm")
```

```{r}
ggp <- ggplot(gnomAD_sum, aes(`AMF Homoplasmic AF`, AMF)) +            
  geom_point(size = 2) 
 

rr = signif(cor(gnomAD_sum$`AMF Homoplasmic AF`, gnomAD_sum$AMF, 
                                use = "pairwise.complete.obs"), 3)

tm.lm = summary(lm(AMF~ -1 + `AMF Homoplasmic AF`, data = gnomAD_sum))
slope = signif(tm.lm$coefficients[1,1], 3)
...
ggp +                                      
  ...
  annotate(geom = "text", x = 3, y = 0.7,
           ...
           size = 5) +
  theme_classic() 
ggsave("cor_MAF_gnomAD_AMR.pdf", width = 20, height = 10, units = "cm")
```

```{r}
library(ggrepel)
dat <- gnomAD_sum %>% select(BP, `EAS Homoplasmic AF`, EAS) %>% 
  filter(!is.na(`EAS Homoplasmic AF`)) %>% 
  arrange(BP) %>%
  mutate(`EAS Homoplasmic AF` = as.numeric(`EAS Homoplasmic AF`),
         EAS = as.numeric(EAS),
         BP = paste("MT", BP, sep = ""),
         `MAF < 0.1%` = ifelse(EAS > 3, "Yes", "No"),
         diff = abs(`EAS Homoplasmic AF` - EAS),
         Name = ifelse(EAS <= 3 & diff > 1.5, BP, "")) %>%
  rename(gnomAD = `EAS Homoplasmic AF`, 
         MVP = EAS,
         `Absolute difference in MAF` = diff) 
...
ggsave("compare_MAF_gnomAD_EAS.pdf", width = 20, height = 10, units = "cm")
```

```{r}
ggp <- ggplot(gnomAD_sum, aes(`EAS Homoplasmic AF`, EAS)) +            
  geom_point(size = 2) 
 

rr = signif(cor(gnomAD_sum$`EAS Homoplasmic AF`, gnomAD_sum$EAS, 
                                use = "pairwise.complete.obs"), 3)

tm.lm = summary(lm(EAS~ -1 + `EAS Homoplasmic AF`, data = gnomAD_sum))
slope = signif(tm.lm$coefficients[1,1], 3)
...
ggp +                                      
  ...
  annotate(geom = "text", x = 3, y = 0.7,
           ...
           size = 5) +
  theme_classic() 
ggsave("cor_MAF_gnomAD_EAS.pdf", width = 20, height = 10, units = "cm")
```
...
## Summplementary Table 8: Summary of Number of markers per set 
```{r}
